...
exports.handler=async function(event){
 try{
   ...
   // Netlify의 :splat은 /published/ 뒤쪽만 넘깁니다.
   // 공개 Blob은 전체 urlPath(/published/...)를 키로 저장했으므로 접두사를 복원합니다.
   if(!path.startsWith('/published/')) path='/published'+path;
   ...
   if(!p){
     try{
       ...
       p=await pages.get(`page/${encodeURIComponent(decoded)}`,{type:'json'});
     }catch(e){}
   }
